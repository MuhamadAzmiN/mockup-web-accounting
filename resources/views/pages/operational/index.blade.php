<x-app-layout>
    <section class="container px-4 mx-auto mt-8">
        ...
        <div class="sm:flex sm:items-center sm:justify-between">
            <div>
                <div class="flex items-center gap-x-3">
                    <h2 class="text-lg font-medium text-gray-800 dark:text-white">Products Operational</h2>
                    <span class="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">
                        {{ $products->total() }} Products
                    </span>
                </div>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">Manage your product operational</p>
            </div>

            <div class="flex items-center mt-4 gap-x-3">
                <a href="{{ route('operational.create') }}" class="flex items-center justify-center w-1/2 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 bg-blue-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600">
        ...
        <div class="mt-6 md:flex md:items-center md:justify-between">
            <form action="{{ route('operational') }}" method="GET" class="relative flex items-center mt-4 md:mt-0 space-x-2">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200">
                                    <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                        <div class="flex items-center gap-x-3">
                                            ...
                                            <div>
                                                <h2 class="font-medium text-gray-800 dark:text-white">{{ $product->product_name }}</h2>
                                    ...
                                    <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                        <div class="flex items-center gap-x-3">
                                            <div>
                                                <h2 class="font-medium text-gray-800 dark:text-white">{{ $product->product_code }}</h2>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                        <div class="flex items-center gap-x-3">
                                            <div>
                                                <h2 class="font-medium text-gray-800 dark:text-white">{{ $product->external_product_code }}</h2>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 text-sm whitespace-nowrap">
                                        <div class="flex items-center gap-x-2">
                                            @if($product->stock > 10)
                                                ...
                                                <span class="text-green-600 dark:text-green-400">{{ $product->stock }} in stock</span>
                                            @elseif($product->stock > 0)
                                                <div class="w-2 h-2 bg-yellow-500 rounded-full"></div>
                                                <span class="text-yellow-600 dark:text-yellow-400">{{ $product->stock }} low stock</span>
                                            @else
                                    ...
                                    <td class="px-4 py-4 text-sm whitespace-nowrap">
                                        <span class="font-medium text-gray-700 dark:text-gray-200">{{ $product->selling_price }}</span>
                                    </td>
                                    <td class="px-4 py-4 text-sm whitespace-nowrap">
                                        <span class="font-medium text-gray-700 dark:text-gray-200">{{ $product->branch->name }}</span>
                                    </td>
                                    <td class="px-4 py-4 text-sm whitespace-nowrap">
                                        <p class="text-gray-500 dark:text-gray-400 max-w-xs truncate">
                                            {{ $product->information ?? 'No description' }}
                                        </p>
                                    </td>
                                    <td class="px-4 py-4 text-sm whitespace-nowrap">
                                        <span class="text-gray-500 dark:text-gray-400">{{ $product->created_at->format('d M Y') }}</span>
                                    </td>
                                    <td class="px-4 py-4 text-sm whitespace-nowrap">
                                        <div class="flex items-center gap-x-2">
                                            <a href="{{ route('operational.show', ["id" => $product->id]) }}" class="text-blue-500 transition-colors duration-200 hover:text-blue-600 dark:hover:text-blue-400">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                </svg>
                                            </a>
                                             <a href="{{ route('operational.edit', ["id" => $product->id]) }}" class="text-gray-500 transition-colors duration-200 hover:text-yellow-500 dark:hover:text-yellow-400">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                                </svg>
                                            </a>
                        
                                            <button onclick="openDeleteModal('{{ $product->id }}', '{{ $product->product_name }}')" class="text-gray-500 transition-colors duration-200 hover:text-red-500 dark:hover:text-red-400">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        ...
        <div class="mt-6 sm:flex sm:items-center sm:justify-between">
            <div class="text-sm text-gray-500 dark:text-gray-400">
                Showing <span class="font-medium text-gray-700 dark:text-gray-100">{{ $products->firstItem() }}</span> to 
                <span class="font-medium text-gray-700 dark:text-gray-100">{{ $products->lastItem() }}</span> of 
                <span class="font-medium text-gray-700 dark:text-gray-100">{{ $products->total() }}</span> results
            </div>
...
function openDeleteModal(productId, productName) {
    ...
    document.getElementById('deleteForm').action = `/operational/${productId}`;
